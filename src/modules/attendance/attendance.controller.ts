...
import {
  ...
  attendanceTeacherParamsSchema,
  bulkAttendanceSchema,
  updateAttendanceSchema,
} from "./attendance.schemas.js";
import { attendanceService } from "./attendance.service.js";

export const listAttendance: RequestHandler = async (request, response) => {
...
export const saveBulkAttendance: RequestHandler = async (request, response) => {
  const input = bulkAttendanceSchema.parse(request.body);
  const result = await attendanceService.save(
    request.auth!,
    input.date,
    input.records,
    input.confirmPublishedFixtureImpact,
  );
  sendSuccess(response, result, "Attendance exceptions saved");
};

export const updateAttendance: RequestHandler = async (request, response) => {
  ...
  sendSuccess(
    ...
    "Attendance exception updated",
  );
};

export const deleteAttendance: RequestHandler = async (request, response) => {
  const { teacherId } = attendanceTeacherParamsSchema.parse(request.params);
  const { date } = attendanceQuerySchema.parse(request.query);
