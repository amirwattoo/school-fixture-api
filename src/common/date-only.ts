...
export const weekdayForDate = (date: Date): DayOfWeek =>
  DAYS_BY_UTC_INDEX[date.getUTCDay()]!;
 ...
 */
export const resolveSchoolDate = (
  value: string,
  timezone: string,
): ResolvedSchoolDate => {
  try {
    ...
    throw new ApiError(
      500,
      "INVALID_SCHOOL_TIMEZONE",
      `The configured school timezone "${timezone}" is invalid`,
    );
  }
