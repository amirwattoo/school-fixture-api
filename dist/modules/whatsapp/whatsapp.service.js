...
import { buildWhatsAppClickToChatUrl, normalizePakistaniWhatsAppNumber, } from "./whatsapp-number.util.js";
import { whatsappRepository, } from "./whatsapp.repository.js";
const clickToChatDetails = (notification) => {
    ...
    try {
        const normalizedDestination = normalizePakistaniWhatsAppNumber(number) ?? "";
        const clickToChatUrl = buildWhatsAppClickToChatUrl(normalizedDestination, notification.message);
...
const findNotification = async (schoolId, notificationId) => {
    const notification = await whatsappRepository.find(schoolId, notificationId);
    if (!notification) {
        throw new ApiError(404, "WHATSAPP_NOTIFICATION_NOT_FOUND", "WhatsApp notification was not found");
    }
    return notification;
};
export const whatsappService = {
    ...
    async get(schoolId, notificationId) {
    ...
    async markOpened(notificationId, actor) {
        ...
        if (details.clickToChatError) {
            throw new ApiError(422, details.clickToChatError.code, details.clickToChatError.message);
        }
        if (existing.status === "MANUALLY_CONFIRMED") {
            return safeNotification(existing);
        }
        const opened = await whatsappRepository.markOpened(existing.id, details.normalizedDestination);
        await createAuditLog(actor, "WHATSAPP_CLICK_TO_CHAT_OPENED", "WhatsAppNotification", opened.id, {
    ...
    async markManuallyConfirmed(notificationId, actor) {
        const existing = await findNotification(actor.schoolId, notificationId);
        if (existing.status === "MANUALLY_CONFIRMED") {
            return safeNotification(existing);
        }
        const confirmed = await whatsappRepository.markManuallyConfirmed(existing.id);
        await createAuditLog(actor, "WHATSAPP_MANUALLY_CONFIRMED", "WhatsAppNotification", confirmed.id, {
