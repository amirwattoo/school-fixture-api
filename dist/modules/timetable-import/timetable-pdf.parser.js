...
import { OFFICIAL_CLASSES, OFFICIAL_SUBJECTS, OFFICIAL_TEACHER_NAMES, PDF_TEACHER_ALIASES, SUBJECT_ALIASES, } from "./timetable-import.constants.js";
const DAY_BY_NUMBER = {
    1: "MONDAY",
    2: "TUESDAY",
    3: "WEDNESDAY",
...
export const expandDayExpression = (expression) => {
    const numbers = [];
    for (const part of expression.replace(/\s+/g, "").split(",")) {
        if (!part)
            throw new Error(`Invalid empty day in "${expression}"`);
        const range = /^([1-5])-([1-5])$/.exec(part);
        if (range) {
            const start = Number(range[1]);
            const end = Number(range[2]);
            if (start > end)
                throw new Error(`Invalid day range "${part}"`);
            for (let day = start; day <= end; day += 1)
                numbers.push(day);
            continue;
        }
        if (!/^[1-5]$/.test(part))
            throw new Error(`Invalid day value "${part}"`);
...
export const normalizeTeacherKey = (value) => normalizeSpaces(value)
    ...
    .replace(/\s+/g, " ")
    .trim();
const officialTeacherByKey = new Map(OFFICIAL_TEACHER_NAMES.map((name) => [normalizeTeacherKey(name), name]));
const teacherAliasByKey = new Map(Object.entries(PDF_TEACHER_ALIASES).map(([alias, name]) => [
...
const teachersOnLine = (line) => {
    ...
    for (const candidate of searchableTeacherNames) {
        ...
        while (index >= 0) {
            if (!matches.some((match) => index < match.index + match.length &&
...
export const resolveTeacherName = (value) => officialTeacherByKey.get(normalizeTeacherKey(value)) ??
    teacherAliasByKey.get(normalizeTeacherKey(value));
export const normalizeSubjectCode = (value) => {
    ...
    return canonicalCodes.has(normalized)
        ? normalized
        : undefined;
};
...
const pdfTextExecutable = () => {
    ...
    throw new Error("pdftotext is required. Set PDFTOTEXT_PATH to the Xpdf executable.");
};
export const extractClassTimetableText = (pdfPath) => {
    const result = spawnSync(pdfTextExecutable(), ["-f", "1", "-l", "3", "-table", "-enc", "UTF-8", pdfPath, "-"], {
        encoding: "utf8",
        windowsHide: true,
        maxBuffer: 10 * 1024 * 1024,
    });
    if (result.status !== 0) {
        throw new Error(`PDF extraction failed: ${normalizeSpaces(result.stderr || "unknown error")}`);
    }
    return result.stdout;
};
const subjectPattern = /^(.+?)\s*\(([\d,\-\s]+)\)$/;
const classPattern = /^\s*((?:[1-9]|1[0-2])[A-B]?|HIFZ)(?=\s|$)/;
export const parseClassTimetableText = (text) => {
    const records = [];
    ...
    pages.forEach((page, pageIndex) => {
        ...
        const headerIndex = lines.findIndex((line) => line.includes("Class") && line.includes("Lesson 8"));
        if (headerIndex < 0)
            return;
        sourcePages.push(sourcePage);
        const header = lines[headerIndex];
        const columnStarts = [...header.matchAll(/Lesson\s+[1-8]/g)].map((match) => match.index);
        if (columnStarts.length !== 8) {
            throw new Error(`Expected eight lesson columns on PDF page ${sourcePage}`);
        }
        ...
        classRows.forEach((row, rowIndex) => {
            ...
            const footerIndex = content.findIndex((line, index) => index > row.index &&
                (line.includes("Timetable Incharge") ||
                    line.includes("Principal,") ||
                    line.includes("©")));
            const end = footerIndex >= 0
            ...
            const className = OFFICIAL_CLASSES.find((item) => item.key === classKey)?.name ??
                classKey;
            const tokensByLesson = columnStarts.map(() => []);
            for (const line of block) {
                ...
                if (teacherMatches.length) {
                    for (const match of teacherMatches) {
                        ...
                        const pendingLessons = tokensByLesson
                            ...
                            .filter((item) => item.last !== undefined && subjectPattern.test(item.last))
                            .sort((left, right) => Math.abs(left.index - nominalLesson) -
                            Math.abs(right.index - nominalLesson));
                        const targetLesson = pendingLessons.find((item) => item.index === nominalLesson)
                ...
                columnStarts.forEach((start, lessonIndex) => {
                    ...
                    const token = normalizeSpaces(finish === undefined
                        ? line.slice(start)
...
//# sourceMappingURL=timetable-pdf.parser.js.map