...
import { isoWeek, parseDateOnly, resolveSchoolDate, weekdayForDate, } from "../../common/date-only.js";
import { env } from "../../config/env.js";
import { isTeacherAvailableAtPeriod } from "../attendance/attendance-availability.js";
import { fixtureNotificationIdempotencyKey, renderFixtureWhatsAppMessage, } from "../whatsapp/whatsapp-message.service.js";
...
const audit = (database, actor, action, entityType, entityId, details) => database.auditLog.create({
    data: {
...
export const fixturesService = {
    ...
    async generate(actor, dateValue, absentTeacherIds) {
        return runSerializable(async (database) => {
            const school = await fixturesRepository.schoolSettings(database, actor.schoolId);
            if (!school) {
            ...
            if (dayOfWeek === "SATURDAY" || dayOfWeek === "SUNDAY") {
                throw new ApiError(400, "NON_WORKING_DAY", `Fixtures cannot be generated for ${dayOfWeek.toLocaleLowerCase("en")}; select a Monday-to-Friday working date`, {
            ...
            const [teachers, attendance, diagnosticTeachers] = await Promise.all([
                fixturesRepository.absentTeachers(database, actor.schoolId, absentTeacherIds),
                fixturesRepository.attendance(database, actor.schoolId, date, absentTeacherIds),
                env.NODE_ENV === "development"
                    ? fixturesRepository.teacherDiagnostics(database, absentTeacherIds)
                    : Promise.resolve([]),
            ]);
            if (env.NODE_ENV === "development") {
                const diagnosticById = new Map(diagnosticTeachers.map((teacher) => [teacher.id, teacher]));
            ...
            if (teachers.length !== absentTeacherIds.length ||
                teachers.some((teacher) => !teacher.isActive)) {
                throw new ApiError(400, "INVALID_ABSENT_TEACHER", "Every absent teacher must belong to your school");
            }
            const attendanceByTeacher = new Map(attendance.map((record) => [record.teacherId, record]));
            const unavailableTeacherCount = attendance.filter((record) => record.status !== "PRESENT").length;
            if (absentTeacherIds.some((teacherId) => {
                const exception = attendanceByTeacher.get(teacherId);
                return !exception || exception.status === "PRESENT";
            })) {
                throw new ApiError(400, "TEACHER_HAS_NO_ATTENDANCE_EXCEPTION", "Every selected teacher must have an attendance exception");
            }
            const matchingLectures = await fixturesRepository.absentLectures(database, actor.schoolId, dayOfWeek, absentTeacherIds);
            const lectures = matchingLectures.filter((lecture) => !isTeacherAvailableAtPeriod(attendanceByTeacher.get(lecture.teacherId), lecture.periodNumber));
            const skippedReasons = [];
            if (matchingLectures.length === 0) {
                skippedReasons.push(`No ${dayOfWeek} timetable periods matched the selected unavailable teachers`);
            }
            const availablePeriodCount = matchingLectures.length - lectures.length;
            if (availablePeriodCount > 0) {
                skippedReasons.push(`${availablePeriodCount} timetable period(s) were skipped because the teacher was available during those periods`);
            }
            const fixtures = [];
            ...
            for (const lecture of lectures) {
                const existing = await fixturesRepository.existingForLecture(database, actor.schoolId, date, lecture);
                if (existing) {
                ...
                if (!selected)
                    fixturesWithoutEligibleReplacementCount += 1;
                if (selected) {
                    const { year, weekNumber } = isoWeek(date);
                    await fixturesRepository.incrementSummary(database, actor.schoolId, selected.teacherId, year, weekNumber);
                    await audit(database, actor, "FIXTURE_AUTO_ASSIGNED", "ProxyFixture", fixture.id, {
                        assignedTeacherId: selected.teacherId,
                        score: selected.totalScore,
                    });
                }
                else {
                    await audit(database, actor, "FIXTURE_UNASSIGNED", "ProxyFixture", fixture.id, { periodNumber: lecture.periodNumber });
                }
                fixtures.push(fixture);
            }
            if (existingFixtureCount > 0) {
                skippedReasons.push(`${existingFixtureCount} existing fixture(s), including published fixtures, were returned without creating duplicates`);
            }
            ...
            await audit(database, actor, "FIXTURES_GENERATED", "ProxyFixture", undefined, {
                date: dateValue,
                absentTeacherIds,
    ...
    async override(actor, fixtureId, assignedTeacherId, reason) {
        return runSerializable(async (database) => {
            const fixture = await fixturesRepository.findForUpdate(database, actor.schoolId, fixtureId);
            if (!fixture)
                throw new ApiError(404, "FIXTURE_NOT_FOUND", "Fixture was not found");
            if (fixture.status !== "DRAFT")
                throw new ApiError(409, "FIXTURE_NOT_DRAFT", "Only draft fixtures can be overridden");
            const eligibility = await getEligibleTeachersForFixture(database, {
                schoolId: actor.schoolId,
                date: fixture.date,
            ...
            const candidate = eligibility.candidates.find((item) => item.teacherId === assignedTeacherId);
            if (!candidate)
                throw new ApiError(400, "INELIGIBLE_OVERRIDE_TEACHER", "The selected teacher is not eligible and free for this period");
            const { year, weekNumber } = isoWeek(fixture.date);
            const sameTeacher = fixture.assignedTeacherId === assignedTeacherId;
            if (fixture.assignedTeacherId &&
                fixture.workloadCounted &&
                !sameTeacher) {
                await fixturesRepository.decrementSummary(database, actor.schoolId, fixture.assignedTeacherId, year, weekNumber);
            }
            if (!sameTeacher || !fixture.workloadCounted) {
                await fixturesRepository.incrementSummary(database, actor.schoolId, assignedTeacherId, year, weekNumber);
            }
            ...
            await audit(database, actor, "FIXTURE_OVERRIDDEN", "ProxyFixture", fixture.id, {
    ...
    async candidates(schoolId, fixtureId) {
        return fixturesRepository.transaction(async (database) => {
            const fixture = await fixturesRepository.findForUpdate(database, schoolId, fixtureId);
            if (!fixture)
                throw new ApiError(404, "FIXTURE_NOT_FOUND", "Fixture was not found");
    ...
    async scoring(schoolId, fixtureId) {
        return fixturesRepository.transaction(async (database) => {
            const fixture = await fixturesRepository.findForUpdate(database, schoolId, fixtureId);
            if (!fixture)
                throw new ApiError(404, "FIXTURE_NOT_FOUND", "Fixture was not found");
    ...
    async cancel(actor, fixtureId) {
        return runSerializable(async (database) => {
            const fixture = await fixturesRepository.findForUpdate(database, actor.schoolId, fixtureId);
            if (!fixture)
                throw new ApiError(404, "FIXTURE_NOT_FOUND", "Fixture was not found");
            if (fixture.status !== "DRAFT")
                throw new ApiError(409, "FIXTURE_NOT_DRAFT", "Only draft fixtures can be cancelled");
            ...
            await audit(database, actor, "FIXTURE_CANCELLED", "ProxyFixture", fixture.id);
            return updated;
        });
    },
    async publish(actor, dateValue) {
        ...
        const publication = await runSerializable(async (database) => {
            ...
            if (drafts.some((fixture) => !fixture.assignedTeacherId)) {
                throw new ApiError(409, "UNASSIGNED_FIXTURES", "Assign every draft fixture before publishing");
            }
            if (drafts.some((fixture) => fixture.requiresReassignment)) {
                throw new ApiError(409, "FIXTURES_REQUIRE_REASSIGNMENT", "Reassign fixtures affected by attendance changes before publishing");
            }
            ...
            await audit(database, actor, "FIXTURES_PUBLISHED", "ProxyFixture", undefined, { date: dateValue, fixtureIds: drafts.map((fixture) => fixture.id) });
            const notificationIds = [];
            let notificationsCreated = 0;
            let existingNotifications = 0;
            for (const fixture of drafts) {
                const assignedTeacher = fixture.assignedTeacher;
                const idempotencyKey = fixtureNotificationIdempotencyKey(fixture.id, assignedTeacher.id, fixture.assignmentVersion);
                ...
                await audit(database, actor, "WHATSAPP_NOTIFICATION_CREATED", "WhatsAppNotification", notification.id, {
